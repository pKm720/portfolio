...
void main() {
  float invPixelRes = 1.0 / max(1.0, uPixelResolution);
  float pixelSize = max(1.0, floor(0.5 + uResolution.x * invPixelRes));
  float invPixelSize = 1.0 / pixelSize;
  
  vec2 fragCoord = floor(gl_FragCoord.xy * invPixelSize);
  vec2 res = uResolution * invPixelSize;
  float invResX = 1.0 / max(1.0, res.x);
...
export default function PixelSnow({
  ...
  useEffect(() => {
    ...
    const getDimensions = () => {
      ...
      const h = container.offsetHeight || container.parentElement?.offsetHeight || 400;
      return { w, h };
    };

    const { w: initW, h: initH } = getDimensions();
    ...
    const material = new ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new Vector2(initW, initH) },
